...
raw.dir <- file.path(raw.dir, "UCI HAR Dataset")
...
#' 
activity.lables <- read.table(file.path(raw.dir, "activity_labels.txt"),
...
#'
#' it is called repeatedly in `read.data` function to extract elements from all
#' lines of a dataset( either train or test)
#' 
...
#' 
#' This function allows reading each dataset separately.
#' it returns a maxtrix of selected columns that is specified by 'col.ids'
#' parameter. 
#' 
...
#' 
#' Input files is fixed-width-formatted data. it can be read with read.fwf() 
#' function, but I prefer to use readLines() for better speed.
#' 
...
train.subjects <- read.misc("subjects", "train")
...
# Done everything remove unpacked raw data
unlink(x=raw.dir, recursive= TRUE)
# cleanup variables 
rm(list=ls())